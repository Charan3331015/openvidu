...
class OpenViduLayout {
 ...
 filterDisplayNone(element) {
  ...
  updateLayout() {
    ...
    const bigOnes = Array.prototype.filter.call( //1. get all likely bigCLass containers, 2. filter out those hiddn, we want only the visible ones (display !== none)
      this.layoutContainer.querySelectorAll('#' + id + '>.' + this.opts.bigClass), //select HTML elem with '#': id=id; '>':that has parent; '.':class=OT_big
  ...
  initLayoutContainer(container, opts) {
    this.opts = {  //<<<-- this is layout options obj
      ...
      bigPercentage: opts.bigPercentage != null ? opts.bigPercentage : 0.8,  //big is = TOT_SIZE_LAYOUT_WINDOW * 0.8
    ...
    this.layoutContainer = typeof container === 'string' ? $(container) : container;   //$(container) search for HTML tag in var container 
  }
